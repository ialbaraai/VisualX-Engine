		...
		if (designer->p_Renderer)
		{
			...
			while (is_running)
			{
				...
				if (Target)
				{
					...
					strncpy_s(NameBuff, Target->Get_Name().c_str(), sizeof(NameBuff) - 1);
					...
					if (Target->Get_Sprite().IsImage)
					{
						...
						{
							strncpy_s(NewEntityImageFilepathBuff, Target->Get_Sprite().FilePath.c_str(), sizeof(NewEntityImageFilepathBuff));
						}
				...
				if (IsCreateWindow)
				{
					...
					ImGui::Begin("Create Object Window", &IsCreateWindow, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoDocking);
				...
				if (IsAddScriptWindow)
				{
					...
					ImGui::Begin("Add Lua Script", &IsAddScriptWindow, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoDocking);
